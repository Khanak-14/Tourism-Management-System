...
public class BookPackage extends JFrame implements ActionListener {
    ...
    BookPackage(String user) {
        ...
        // Fetching package names from the database
        try {
            Conns c = new Conns();
            ResultSet rs = c.stmt.executeQuery("SELECT name FROM packages");
            while (rs.next()) {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            ...
            // Fetching package price from the database
            try {
                Conns c = new Conns();
                ResultSet rs = c.stmt.executeQuery("SELECT cost FROM packages WHERE name = '" + pck + "'");
                if (rs.next()) {
                    cost = rs.getInt("cost");
                }
            } catch (Exception e) {
